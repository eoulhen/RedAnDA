...
RC=xar.load_dataset('ISAS_TropicalPacific.nc').isel(depth=[0,1,2,3,4,5,6,7])#.isel(depth=[0,1,2,3,5,7])#,6,7,8])
...
Alphabet=xar.load_dataset('ISAS_AlphaBetaCoeffs.nc').rename({"ALPH":"alpha","BETA":"beta"}).isel(depth=[0,1,2,3,4,5,6,7]).assign_coords({'latitude':RC.latitude})
...
ENtemp2 = xar.load_dataset(glob.glob('ENtemp_NoInterp*0NoFMoorAdjusted.nc')[0]).isel(depth=[0,1,2,3,4,5,6,7])#.isel(depth=[0,3,4,5,6,7,8])
for i in range(0,7):
...
Robs=xar.load_dataset('Robs_TEMP.nc').drop(('time','time2')).isel(depth=[0,1,2,3,4,5,6,7])
Robs= Robs.where(Robs['POTM_ERR']!=np.inf,np.nan)
Robs2=xar.load_dataset('Robs_PSAL.nc').drop(('time','time2')).isel(depth=[0,1,2,3,4,5,6,7])
Robs2= Robs2.where(Robs2['PSAL_ERR']!=np.inf,np.nan)
...
A1=np.concatenate((AT*np.mean(wAT.values.reshape((-1,NxNyNz)),0)[np.newaxis].T,AS*np.mean(wAS.values.reshape((-1,NxNyNz)),0)[np.newaxis].T))
...
#             loglik = np.load(savefile,allow_pickle=True)[3]
    ...
    if (method =='AnEnKF' or method =='AnEnKS'):
        for k in tqdm(range(k0,T)):
            ...
            InstrErr2 = xar.concat(((_1990['ErrT']).where(_1990['POTM'].notnull().values).dropna(dim='ALL'),(_1990['ErrS']).where(_1990['PSAL'].notnull().values).dropna(dim='ALL')),dim='ALL')
...
x_hat_analog = AnDA_data_assimilation(yo, AF, RC, 1*Robs.rolling({'latitude':3,'longitude':3},center=True,min_periods=4).mean().where(Robs['PSAL_ERR'].notnull()), Xb[::-1], Btime[1:][::-1] , N=250, method ='AnEnKS',savefile='T&S_savefile2.npy',load=False)
...
redAnDA=redAnDA.assign({"Alphabet":(['var','depth','Lat','Lon'],np.concatenate((Alphabet.ALPH.mean('time').values[np.newaxis],Alphabet.BETA.mean('time').values[np.newaxis]),axis=0))})
...
redAnDA.to_netcdf('redAnDA_T&S_'+str(nEOF)+'modes.nc','w') 
