...
A1=(RC['PSAL_detrend'].fillna(0)).stack(NxNyNz=['depth','latitude','longitude']).values.T
C=RC['PSAL'].fillna(0).std('time').stack(NxNyNz=['depth','latitude','longitude']).values
    ...
    if (method =='OI'):
        for k in tqdm(range(k0,T)):
            ...
            if (8*(_obs['ALL'].size**2))/((2**30) *1)<1:
                Ro=da.array.diag((H.dot((Robs['PSAL_ERR'].fillna(0).stack(NxNyNz=['depth','latitude','longitude']))**2)))
...
Btime=np.append(Btime,np.datetime64(str(timemax+1)+'-'+str(1).zfill(2)+'-15'),)
# nEOF=180 # 70
Na = len(Btime)Jtime=np.asanyarray((Btime-np.datetime64('1950-01-01')).astype('timedelta64[D]').astype('float64'))
...
#START THE ANALYSIS
x_hat_oi = OI(yo, RC, Robs.rolling({'latitude':3,'longitude':3},center=True,min_periods=4).mean().where(Robs['PSAL_ERR'].notnull()), Xb[:], Btime[:-1],savefile='oisavefile.npy',load=False )
...
OI['values'].attrs={'units': 'PSS', 'description': 'Analysed Absolute Practical Salinity Field'}
OI.uncert.attrs={'units': 'PSS^2', 'description': 'Uncertainty of the analysis (variance)'}
...
OI.to_netcdf('OI_PSAL_OCCIPUT.nc','w') 
