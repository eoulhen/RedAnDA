...
RC=xar.open_mfdataset('ISAS_TropicalPacific.nc').isel(depth=[0,1,2,3,4,5,6,7])
...
#LOAD OBSERVATIONS
ENtemp2 = xar.open_mfdataset(glob.glob('ENtemp_NoInterp*0NoFMoorAdjusted.nc')[0]).isel(depth=[0,1,2,3,4,5,6,7])
for i in range(0,7):
    ENtemp2= xar.concat((ENtemp2,xar.open_mfdataset(glob.glob('ENtemp_NoInterp*0NoFMoorAdjusted.nc')[1:][i]).isel(depth=[0,1,2,3,4,5,6,7])),'N_PROF')

ENtemp2=  ENtemp2.where(ENtemp2['POTM']>=0).compute()


#LOAD OBSERVATIONS INSTRUMENTAL ERRORS
InstrErr = xar.open_mfdataset('ENtemp_ERR.nc',).rename({"Err":"ErrT"})['ErrT']

EN= ENtemp2.merge(InstrErr)
...
Robs=xar.open_mfdataset('Robs_TEMP.nc').drop(('time','time2')).isel(depth=[0,1,2,3,4,5,6,7])
Robs= Robs.where(Robs['POTM_ERR']!=np.inf,np.nan)
...
A1=((RC['TEMP_detrend']).fillna(0)*Weights).stack(NxNyNz=['depth','latitude','longitude']).values.T #-np.tile(RC['T_Climato'].values,(192//12,1,1,1))).fillna(0).stack(NxNyNz=['latitude','longitude','depth']).values.T
...
#             loglik = np.zeros([T])
    ...
    if (method =='AnEnKF' or method =='AnEnKS'):
        for k in tqdm(range(k0,T)):
            ...
            Nobs = _1990['POTM'].dropna(dim='ALL')['ALL'].size
            #THE TEMPERATURE OBSERVATIONS OF THE PRESENT TIME STEP ARE SELECTED
            _1990 = (_1990['POTM'].unstack({'ALL'})).stack({'ALL':['N_PROF','depth']}).dropna(dim='ALL')
            Nobs=_1990['ALL'].size
            #INSTRUMENTAL ERRORS ASSOCIATED WITH SELECTED OBSERVATIONS            
            InstrErr2 = (_1990['ErrT']).where(_1990['POTM'].notnull().values).dropna(dim='ALL')
            ...
            if _1990.N_PROF.size>0:
                ...
                H = np.zeros((Nobs,NxNy))                H[np.unravel_index((index_knn+np.arange(0,H.size,H.shape[1])[:,np.newaxis]).ravel(),H.shape)]=((1/dist_knn)/(1/dist_knn).sum(axis=1).reshape((-1,1))).ravel()
                ...
                Erro =((Robs['POTM_ERR'])).fillna(0).stack(NxNyNz=['depth','latitude','longitude'])
                if (8*(_1990['ALL'].size**2))/((2**30) *1)<1: #IF A LOT OF OBS : USE DASK
                    ...
                    Ro=da.array.diag((H).dot(Erro**2))+preRo+np.eye(postRo.shape[0])*postRo

                    del preRo,postRo

                else:
                    preRo=da.array.dot(da.array.dot(H.dot(Undo),eigclim[:,nEOF:scd_trunct].dot(np.diag(R[:scd_trunct-nEOF]).dot(eigclim[:,nEOF:scd_trunct].T))),(H.dot(Undo)).T).rechunk(block_size_limit=5e8,balance=True)

                    postRo=da.array.dot(da.array.dot(H.dot(Undo),eigclim[:,scd_trunct:].dot(np.diag(R[scd_trunct-nEOF:]).dot(eigclim[:,scd_trunct:].T))),(H.dot(Undo)).T).rechunk(preRo.chunks)

                    Ro=(da.array.diag((H).dot(Erro**2))).rechunk(preRo.chunks)+preRo+da.array.eye(postRo.shape[0]).rechunk(block_size_limit=5e8)*postRo
            ...
            if (len(i_var_obs)>0)&(k>0):
                ...
                del Ro,y,Eo,SIGMA,SIGMA_INV,eps,yf,K,d   
            else:
                x_hat.part[k,:,:] = xf
...
class catalog:
    analogs = aclim[1:,:nEOF];
    successors = aclim[:-1,:nEOF];

class AF:
    k =170  #170# number of analogs
...
x_hat_analog = AnDA_data_assimilation(yo, AF, RC, Robs.rolling({'latitude':3,'longitude':3},center=True,min_periods=4).mean().where(Robs['POTM_ERR'].notnull()), Xb[::-1], Btime[1:][::-1] , N=250, method ='AnEnKS',savefile='savefile.npy',load=False)
...
redanda=redanda.drop('lat').drop(('lon','mthcov')).assign({"Climato":(['month','depth','Lat','Lon'],RC.T_Climato.values)})
...
redanda.functions.attrs={'units': 'K', 'description': 'EOFs derived from the learning period temperature field'}
redanda.truncerr.attrs={'units': 'K', 'description': 'Truncation error : Standard deviation associated with non-retained EOFs'}
redanda.Climato.attrs={'units': 'K', 'description': 'Monthly climatology derived from the learning period'+str(RC.time2.dt.year.min().values)+'-'+str(RC.time2.dt.year.max().values)}
...
"-"+str(timemax)+"  analysis, with UnivarT","How to use": " Apply the matrix product of the coeff with the functions to obtain the full grid anomalies.","Coefficient of inflation of the EOF spectrum" :"0.1","Minimal number of obs for analysis":"5000"})


redAnDA.to_netcdf('UnivarT_'+str(nEOF)+'modes.nc','w') 
