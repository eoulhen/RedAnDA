...
# OPEN OBSERVATIONS
ENtemp2=xar.open_mfdataset('ENsal_NoInterpNoFMoorAdjusted.nc').compute()
...
ENtemp2=ENtemp2.where(ENtemp2['PSAL']>0,drop=True)
...
Clim*=0
Xb = xar.DataArray(0*Clim.fillna(0).data+RC['S_Climato'].coarsen({'latitude':didivy,'longitude':didivx},boundary="pad").mean(skipna=True).stack(NxNy=['latitude','longitude']).fillna(0).data[np.linspace(0,Na-1,Na,dtype='int')%12],coords=[np.arange(0,Na,1),RC['depth'],np.arange(0,NxNy,1)],dims=['Time','depth','NxNy'])
...
#Anomalies
Tano = ENtemp2['PSAL'].sel({'depth':i}).dropna(dim='N_PROF').values-(H.dot(Xb[:,i].T)*G).sum(1)
...
# Robs file is saved
Robs.rename('PSAL_ERR').to_netcdf('Robs_PSAL.nc','w') 
