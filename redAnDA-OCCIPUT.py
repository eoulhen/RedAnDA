...
Robs=xar.load_dataset('Robs_TEMP_OCCIPUT.nc')
...
A1=((RC['TEMP_detrend']).fillna(0)*Weights).stack(NxNyNz=['depth','latitude','longitude']).values.T 
...
#GET RID OF OBSERVATIONS THAT FALL OUTSIDE THE GRID
            ...
            y= (_1990['POTM']-(H.dot(Xb.stack({'NxNyNz':['depth','NxNy']}).values.T)*G).sum(1)).values[outofbound]
...
x_hat_analog = AnDA_data_assimilation(yo, AF, RC, Robs.rolling({'latitude':3,'longitude':3},center=True,min_periods=4).mean().where(Robs['POTM_ERR'].notnull())['POTM_ERR'], Xb[::-1], Btime[1:][::-1] , N=250, method ='AnEnKS',savefile='OCCIPUT_savefile.npy',load=False)
...
"-"+str(timemax)+" analysis, with UnivarT","How to use": " Apply the matrix product of the coeff with the functions to obtain the full grid anomalies.","Coefficient of inflation of the EOF spectrum" :"0.5","Minimal number of obs for analysis":"5000"})

redAnDA.to_netcdf('UnivarT_'+str(nEOF)+'modes_OCCIPUT.nc','w') 
